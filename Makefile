...
DIST_COMMON = README $(am__configure_deps) $(pkginclude_HEADERS) \
	...
	$(srcdir)/irssi-config.h.in $(srcdir)/irssi-config.in \
	...
	ChangeLog INSTALL NEWS TODO acconfig.h build-aux/config.guess \
...
ACLOCAL = ${SHELL} /Users/notrael/irssi-0.8.15/build-aux/missing --run aclocal-1.11
AMTAR = ${SHELL} /Users/notrael/irssi-0.8.15/build-aux/missing --run tar
AR = ar
AUTOCONF = ${SHELL} /Users/notrael/irssi-0.8.15/build-aux/missing --run autoconf
AUTOHEADER = ${SHELL} /Users/notrael/irssi-0.8.15/build-aux/missing --run autoheader
AUTOMAKE = ${SHELL} /Users/notrael/irssi-0.8.15/build-aux/missing --run automake-1.11
AWK = gawk
CC = gcc
...
GLIB_LIBS = -L/opt/local/lib -lgmodule-2.0 -lglib-2.0 -lintl -liconv  
...
LTLIBOBJS = 
MAINT = #
MAKEINFO = ${SHELL} /Users/notrael/irssi-0.8.15/build-aux/missing --run makeinfo
...
PATH_SEPARATOR = :
PERL_CFLAGS = -pipe -O2 -arch x86_64 -fno-common -DPERL_DARWIN -I/opt/local/include -no-cpp-precomp -fno-strict-aliasing -I/usr/local/include -I/opt/local/include -I/opt/local/lib/perl5/5.8.9/darwin-2level/CORE
PERL_EXTRA_OPTS = CCCDLFLAGS="-fPIC"
PERL_FE_LINK_LIBS = ../perl/libfe_perl_static.la
PERL_LDFLAGS = 
PERL_LINK_FLAGS = -L/opt/local/lib -arch x86_64 -L/usr/local/lib -L/opt/local/lib/perl5/5.8.9/darwin-2level/CORE -lperl -ldl -lm -lutil
PERL_LINK_LIBS = ../perl/libperl_core_static.la
PERL_MM_PARAMS = 
PERL_STATIC_LIBS = 0
PERL_USE_LIB = 
PKG_CONFIG = /opt/local/bin/pkg-config
PROG_LIBS =  -L/opt/local/lib -lgmodule-2.0 -lglib-2.0 -lintl -liconv   -lssl -lcrypto
RANLIB = ranlib
SED = /usr/bin//sed
SET_MAKE = 
SHELL = /bin/sh
...
VERSION = 0.8.15
abs_builddir = /Users/notrael/irssi-0.8.15
abs_srcdir = /Users/notrael/irssi-0.8.15
abs_top_builddir = /Users/notrael/irssi-0.8.15
abs_top_srcdir = /Users/notrael/irssi-0.8.15
ac_ct_CC = gcc
ac_ct_DUMPBIN = 
am__include = include
am__leading_dot = .
...
install_sh = ${SHELL} /Users/notrael/irssi-0.8.15/build-aux/install-sh
...
all: $(BUILT_SOURCES) irssi-config.h
...
irssi-config.h: stamp-h1
	@if test ! -f $@; then \
	  rm -f stamp-h1; \
	  $(MAKE) $(AM_MAKEFLAGS) stamp-h1; \
	else :; fi

stamp-h1: $(srcdir)/irssi-config.h.in $(top_builddir)/config.status
	@rm -f stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status irssi-config.h
$(srcdir)/irssi-config.h.in: # $(am__configure_deps) $(top_srcdir)/acconfig.h
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f stamp-h1
	touch $@

distclean-hdr:
	-rm -f irssi-config.h stamp-h1
...
TAGS: tags-recursive $(HEADERS) $(SOURCES) irssi-config.h.in $(TAGS_DEPENDENCIES) \
		$(TAGS_FILES) $(LISP)
	set x; \
	here=`pwd`; \
	...
	list='$(SOURCES) $(HEADERS) irssi-config.h.in $(LISP) $(TAGS_FILES)'; \
	unique=`for i in $$list; do \
	    if test -f "$$i"; then echo $$i; else echo $(srcdir)/$$i; fi; \
	  done | \
	  $(AWK) '{ files[$$0] = 1; nonempty = 1; } \
...
ctags: CTAGS
CTAGS: ctags-recursive $(HEADERS) $(SOURCES) irssi-config.h.in $(TAGS_DEPENDENCIES) \
		$(TAGS_FILES) $(LISP)
	list='$(SOURCES) $(HEADERS) irssi-config.h.in $(LISP) $(TAGS_FILES)'; \
	unique=`for i in $$list; do \
	    if test -f "$$i"; then echo $$i; else echo $(srcdir)/$$i; fi; \
...
all-am: Makefile $(DATA) $(HEADERS) irssi-config.h
...
uninstall-am: uninstall-confDATA uninstall-pkgincludeHEADERS \
	uninstall-themeDATA
